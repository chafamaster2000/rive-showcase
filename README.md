...
- **A switch** whose motion lives entirely in a state machine: one layer blends off and on, another scales the knob on hover. React owns the input and the accessibility.
...
Still to author: `button.riv`, `loader.riv`, `kart.riv`. `art/workspace.rev` is a backup of the editor document.
